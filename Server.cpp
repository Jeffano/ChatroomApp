...
const std::string AUTHENTICATION_STRING = "TEAM10_VAILIDATION_STRING";
...
public:
	SocketThread(Socket &socket, std::vector<SocketThread *> &clientSocketThread, bool &exitBool, int port) : socket(socket), socketThreadHolder(clientSocketThread), exitBool(exitBool), port(port), isAuthenticated(false)
	{
...
private:
	bool Authenticate()
	{
		// Read the authentication string from the client
        ByteArray authData;
        socket.Read(authData);
        std::string authString = authData.ToString();

        // Compare with the known authentication string
        return authString == AUTHENTICATION_STRING;
	}
};
...
public:
	...
	~ServerThread()
	{
		...
		for (auto it = sThreadList.begin(); it != sThreadList.end();)
		{
			try
			{
				// Close the socket.
				Socket &toClose = (*it)->GetSocket();
				toClose.Close();
				// Erase the thread from the list.
				delete *it;
				it = sThreadList.erase(it);
			}
			catch (const std::string &error)
			{
				std::cout << "ERROR: " << error << std::endl;
...
int main(void)
{
	...
	std::cout << "Group 10 Server" << std::endl
